...
exports.getAccount = async (req, res, next) => {
  try {
    ...
    // Fetch wishlist tours from Wishlist collection
    ...
    const wishlist = wishlistItems.map(item => item.tour);

    res.status(200).render('account', {
      title: 'Your Account',
      user,
      wishlist, // pass wishlist separately
    });
  } catch (err) {
    next(err);
  }
...
exports.getWishlistPage = async (req, res, next) => {
  try {
    ...
    const tours = wishlistItems.map(item => item.tour);
